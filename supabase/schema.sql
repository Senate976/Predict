...
-- ---------------------------------------------------------------------------
-- 22. Destinataires visibles directement sur la carte du Fil
-- ---------------------------------------------------------------------------
--
-- Jusqu'ici, voir l'audience d'une prédiction exigeait d'ouvrir son écran
-- détail (`prediction_access` via `fetchPredictionRecipients`) — trop discret,
-- le bogue persistant remonté était que cette liste restait invisible pour un
-- destinataire quand la prédiction venait d'un ami. On l'ajoute donc en clair
-- sur `predictions_feed`, dès le premier chargement du fil.
--
-- Sous-requête corrélée plutôt qu'un troisième `left join` direct sur
-- `prediction_access` : cette vue fait déjà un `left join` sur
-- `prediction_votes` (un-à-plusieurs) agrégé par `group by` — un second
-- `left join` un-à-plusieurs sur `prediction_access` multiplierait les lignes
-- de vote avant l'agrégation et fausserait `realized_votes`/`missed_votes`
-- (un même vote compté une fois par destinataire). Le `array_agg` corrélé
-- évite ce piège : il s'exécute une fois par prédiction, indépendamment du
-- `group by` du reste de la vue.
--
-- Tourne avec les droits de l'appelant (`security_invoker` de la vue) : la
-- RLS de `prediction_access` (`prediction_access_select`, section 11) décide
-- seule qui voit quoi — l'auteur voit toute son audience, et tout destinataire
-- voit le reste de l'audience de la même prédiction (déjà étendu en section
-- 11). `coalesce(..., array[]::text[])` : un tableau vide plutôt que `null`
-- quand personne n'a encore accès, plus simple à consommer côté client.
drop view if exists public.predictions_feed;
...
-- ---------------------------------------------------------------------------
-- 23. Filet de sécurité — forcer PostgREST à relire le schéma
-- ---------------------------------------------------------------------------
--
