...
-- ---------------------------------------------------------------------------
-- 26. Invitations par SMS — inviter quelqu'un par son numéro de téléphone
-- ---------------------------------------------------------------------------
--
-- `phone_invites` garde trace de chaque numéro invité par SMS. Quand la
-- personne invitée crée un compte et renseigne ce même numéro dans son
-- profil, un déclencheur transforme automatiquement l'invitation en demande
-- d'ami en attente — l'invité n'a plus qu'à l'accepter depuis « Demandes
-- reçues », comme pour toute autre demande.

-- Uniformise un numéro avant comparaison : enlève espaces/points/tirets, et
-- convertit un 0 initial (format français local) en +33. Reste minimal —
-- suffisant pour cette appli, pas une validation E.164 complète.
...
as $$
  select case
    ...
    when regexp_replace(raw, '[^0-9+]', '', 'g') like '+%'
      then regexp_replace(raw, '[^0-9+]', '', 'g')
    when regexp_replace(raw, '[^0-9+]', '', 'g') like '0%'
      then '+33' || substring(regexp_replace(raw, '[^0-9+]', '', 'g') from 2)
...
create table if not exists public.phone_invites (
  ...
  phone text not null,
  created_at timestamptz not null default now(),
  -- Renseigné une fois l'invitation transformée en demande d'ami — évite de
  -- retraiter la même invitation à chaque nouvelle mise à jour de profil.
  friendship_id uuid references public.friendships (id) on delete set null
);

create index if not exists phone_invites_phone_idx on public.phone_invites (phone);

-- Toujours le numéro sous sa forme normalisée en base, quel que soit le
-- format saisi côté client — c'est ce que le déclencheur ci-dessous compare
-- au numéro (lui aussi normalisé) du profil qui vient de s'inscrire.
create or replace function public.normalize_phone_invite()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists phone_invites_normalize on public.phone_invites;
create trigger phone_invites_normalize
  before insert or update of phone on public.phone_invites
  for each row execute function public.normalize_phone_invite();

alter table public.phone_invites enable row level security;

drop policy if exists "phone_invites_select_own" on public.phone_invites;
create policy "phone_invites_select_own"
  on public.phone_invites
  for select
  to authenticated
  using (inviter_id = auth.uid());

drop policy if exists "phone_invites_insert_own" on public.phone_invites;
create policy "phone_invites_insert_own"
  on public.phone_invites
  for insert
  to authenticated
  with check (inviter_id = auth.uid());

-- La fonction Edge `send-invite-sms` supprime l'invitation qu'elle vient de
-- créer si l'envoi du SMS échoue, pour ne pas garder une invitation fantôme.
drop policy if exists "phone_invites_delete_own" on public.phone_invites;
create policy "phone_invites_delete_own"
  on public.phone_invites
  for delete
  to authenticated
  using (inviter_id = auth.uid());

-- `security definer` : le déclencheur doit pouvoir créer une demande d'ami au
-- nom de l'auteur de l'invitation, un utilisateur différent de celui dont le
-- profil vient d'être créé/modifié (qui n'a par ailleurs pas le droit
-- d'insérer une ligne `friendships` pour quelqu'un d'autre).
create or replace function public.handle_phone_invite_match()
returns trigger
language plpgsql
security definer
...
begin
  ...
  for invite in
    select * from public.phone_invites
  ...
  loop
    if not exists (
      ...
      where (requester_id = invite.inviter_id and addressee_id = new.id)
         or (requester_id = new.id and addressee_id = invite.inviter_id)
    ) then
      insert into public.friendships (requester_id, addressee_id, status)
      values (invite.inviter_id, new.id, 'pending')
      returning id into new_friendship_id;

      update public.phone_invites set friendship_id = new_friendship_id where id = invite.id;
    end if;
  end loop;
...
drop trigger if exists on_profile_phone_invite_match on public.profiles;
create trigger on_profile_phone_invite_match
  after insert or update of phone on public.profiles
  for each row execute function public.handle_phone_invite_match();

-- ---------------------------------------------------------------------------
-- 27. Filet de sécurité — forcer PostgREST à relire le schéma
-- ---------------------------------------------------------------------------
--
