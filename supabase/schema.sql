...
-- ---------------------------------------------------------------------------
-- 33. Retrait des votes Hype (scellé) et Réputation (en cours)
-- ---------------------------------------------------------------------------
--
-- Retour en arrière complet sur la section 32 : les deux jauges de
-- gamification sont retirées de l'app, donc plus besoin de la distinction
-- `vote_type` qu'elles avaient rendue nécessaire — un vote par destinataire
-- suffit à nouveau (realized/missed OU believe/disbelieve selon le type de
...
-- ---------------------------------------------------------------------------
-- 65. La relance se rejoue au bout de sept jours
-- ---------------------------------------------------------------------------
--
-- Une relance par personne et par prédiction, définitivement : c'était la
-- règle, et elle avait un défaut. Le bouton « Impatient » restait sur la carte
-- une fois touché — il fallait bien pouvoir retirer sa relance — et venait
-- donc encombrer une carte scellée à laquelle on n'avait plus rien à dire.
--
-- La relance devient renouvelable au bout de SEPT JOURS. Ce qui change :
-- une fois envoyée, le bouton disparaît de la carte ; il revient une semaine
-- plus tard, et une nouvelle relance rallume la notification de l'auteur.
-- Retirer sa relance n'a plus lieu d'être : elle s'efface d'elle-même.
--
-- Ce que ça NE change pas, et c'est l'essentiel : le compteur reste un nombre
-- de PERSONNES, pas de coups. La ligne est mise à jour, jamais dupliquée — la
-- clé primaire s'en charge. Quelqu'un qui relance chaque semaine pendant six
-- mois compte toujours pour un.
create or replace function public.nudge_prediction(p_prediction_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  -- `where` sur le `do update` : une relance déjà envoyée dans les sept
  -- derniers jours ne compte pas une seconde fois. Sans cette condition, un
  -- appui répété rallumerait la notification de l'auteur en boucle — c'est
  -- très exactement le harcèlement à faible coût qu'on cherche à éviter.
  insert into public.prediction_nudges (prediction_id, sender_id)
  values (p_prediction_id, auth.uid())
  on conflict (prediction_id, sender_id) do update
    set created_at = now()
    where public.prediction_nudges.created_at <= now() - interval '7 days'
  returning true into v_counted;

  -- `v_counted` est nul quand le `where` a écarté la ligne : rien n'a été
  -- inséré ni mis à jour, il n'y a donc rien à notifier.
  if v_counted is null then
    return;
  end if;

  -- Une seule notification par prédiction, rallumée à chaque relance qui
  -- compte plutôt qu'empilée — même mécanique que `generate_open_reminders`.
  insert into public.notifications (user_id, prediction_id, type)
  values (v_author_id, p_prediction_id, 'nudge')
  on conflict (user_id, prediction_id, type)
  do update set created_at = now(), is_read = false, is_dismissed = false;
...
/**
 * Le nombre de personnes qui s'impatientent sur une prédiction.
 *
 * `security definer`, et ce n'est pas un détail : la policy de
 * `prediction_nudges` ne laisse voir à chacun QUE SA PROPRE ligne — c'est
 * elle qui garantit qu'on ne saura jamais qui a relancé. La vue du Fil étant
 * en `security_invoker`, un `count(*)` écrit directement dedans était filtré
 * par cette même policy : il ne valait donc jamais que 0 ou 1, et l'auteur —
 * qui ne relance jamais sa propre prédiction — lisait toujours zéro.
 *
 * Compter ici, hors de la RLS de l'appelant, rend le nombre juste sans rien
 * révéler des noms : la fonction ne renvoie qu'un entier.
 *
 * Le garde-fou reste : on ne compte que si l'appelant a le droit de voir la
 * prédiction, faute de quoi ce compteur renseignerait sur des prédictions
 * auxquelles il n'a pas accès.
 */
create or replace function public.nudge_count(p_prediction_id uuid)
returns integer
language sql
security definer
stable
...
as $$
  select coalesce((
    select count(*)::integer from public.prediction_nudges pn
    where pn.prediction_id = p_prediction_id
      and exists (
        select 1 from public.predictions p
        where p.id = p_prediction_id
          and (p.author_id = auth.uid() or public.has_prediction_access(p.id, auth.uid()))
...
revoke all on function public.nudge_count(uuid) from public;
grant execute on function public.nudge_count(uuid) to authenticated;


-- La vue du Fil, redéfinie pour que `i_nudged` ne parle plus que des sept
-- derniers jours, et pour que le compteur passe par `nudge_count`.
drop view if exists public.predictions_feed;
...
select
  ...
  -- une sommation, et il reste seul à décider du moment.
  --
  -- Par `nudge_count` et non par un `count(*)` écrit ici : la vue est en
  -- `security_invoker`, un décompte direct serait filtré par la policy de
  -- `prediction_nudges` (chacun ne voit que sa propre ligne) et ne vaudrait
  -- jamais plus de 1. Voir la fonction pour le détail.
  public.nudge_count(p.id) as nudge_count,
  -- Ai-je relancé RÉCEMMENT ? Sept jours, la même fenêtre que
  -- `nudge_prediction`. Tant que c'est vrai, le bouton « Impatient »
  -- disparaît de la carte : il n'a rien à y faire, on vient de s'exprimer.
  -- Passé ce délai il revient, et une nouvelle relance rallume la
  -- notification de l'auteur.
  (
    select exists (
      select 1 from public.prediction_nudges pn2
      where pn2.prediction_id = p.id
        and pn2.sender_id = auth.uid()
        and pn2.created_at > now() - interval '7 days'
    )
  ) as i_nudged
from public.predictions p
left join public.prediction_contents pc on pc.prediction_id = p.id;
